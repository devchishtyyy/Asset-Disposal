...
router.post('/verify', authenticate, async (req, res) => {
  ...
  try {
    ...
    if (err.message === 'SAP_UNAUTHORIZED')  return res.status(401).json({ error: 'SAP_UNAUTHORIZED' });
    if (err.message === 'SAP_NETWORK_ERROR') return res.status(502).json({ error: 'SAP_NETWORK_ERROR' });
...
router.post('/asset', authenticate, async (req, res) => {
  ...
  try {
    ...
    if (err.message === 'SAP_NOT_FOUND')     return res.status(404).json({ error: 'SAP_NOT_FOUND' });
    if (err.message === 'SAP_UNAUTHORIZED')  return res.status(401).json({ error: 'SAP_UNAUTHORIZED' });
    if (err.message === 'SAP_NETWORK_ERROR') return res.status(502).json({ error: 'SAP_NETWORK_ERROR' });
    res.status(500).json({ error: 'SAP_NETWORK_ERROR' });
  }
